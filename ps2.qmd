...
```{r}
#testing all three to see how each function reports 
range(d$Game1Angry1, na.rm = TRUE) #1-7 
hist(d$Game1Angry1)
unique(d$Game1Angry1, na.rm = TRUE)
```
...
```{r}
d$DoNotUse
unique(d$DoNotUse) #NA, 1
 filtered_d = d %>% 
  filter(
    is.na(DoNotUse)|DoNotUse != 1) # your code here: exclude subjects that are marked as"DoNotUse"
...
performance_hyp_long_d = performance_hyp_long_d %>% 
  mutate(ConfrontationalGame = grepl("SOF", Measurement), # create a new variable that will say whether the measurement was of the game soldier of fortune (SOF).
         ...
         MusicCondition = 
           factor(ifelse(Cond > 3, Cond-3, Cond), 
                  levels = 1:3, 
                  labels = c("Anger", "Exciting", "Neutral"))) # Get rid of uninterpretable condition labels
  head(performance_hyp_long_d)
```
...
rating_hyp_long_d = rating_hyp_long_d %>%
  ...
  table(rating_hyp_long_d$IsRecall)
```
...
```{r TamirFig1}
emo_fill <- ggplot(rating_summary_d, 
                   aes(x=ConfrontationalGame, 
                       y=MeanRating, 
                       fill=Emotion)) +
...
performance_diff_d = performance_hyp_long_d %>%
  mutate(WithMusic = factor(WithMusic, 
                            levels=c(F, T), 
                            labels=c("PreMusic", "PostMusic"))) %>% # first, tweak the variable so our code is easier to read.
...
If you don't understand every step of that code (or any other `dplyr` code), it can be helpful to look at the result of running just the first line, then just the first two lines, and so on. #this was helpful! 
...
```{r TamirFig2}
ggplot(performance_diff_summary_d, 
       aes(x=ConfrontationalGame, 
